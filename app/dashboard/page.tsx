...
export default function DashboardPage() {
  ...
  if (loading || status === "loading") {
    return (
      <div className="h-auto bg-slate-50 overflow-hidden grid gap-[18px] p-6 px-7" style={{ gridTemplateRows: 'auto auto 1fr' }}>
        <div className="flex items-center justify-between">
          <div>
            <SkeletonBox w="300px" h="30px" radius="8px" />
            <div className="mt-2"><SkeletonBox w="250px" h="14px" radius="4px" /></div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3.5 mt-4">
          {[0, 1, 2].map(i => (
            <div key={i} className="bg-white border border-slate-200 rounded-[14px] p-[18px_20px] flex items-center gap-4">
              <SkeletonBox w="46px" h="46px" radius="12px" />
              <div className="flex-1">
                <SkeletonBox w="60%" h="12px" radius="4px" />
                <div className="mt-2"><SkeletonBox w="40%" h="30px" radius="6px" /></div>
              </div>
            </div>
          ))}
        </div>
        <div className="grid gap-3.5 mt-4" style={{ gridTemplateColumns: '1fr 1fr 290px' }}>
          {[0, 1].map(i => (
            <div key={i} className="bg-white border border-slate-200 rounded-[14px] p-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3.5">
            {[0, 1].map(i => (
              <div key={i} className="bg-white border border-slate-200 rounded-[14px] p-4 flex flex-col gap-4 min-h-[200px]">
   ...
   <div className="h-auto bg-slate-50 overflow-hidden grid gap-[18px] p-6 px-7 animate-fade-in-up" style={{ gridTemplateRows: 'auto auto 1fr' }}>
      ...
      <div className="grid grid-cols-3 gap-3.5">
        ...
        <div className="bg-white border border-slate-200 rounded-[14px] p-[18px_20px] flex items-center gap-4 transition-all duration-[180ms] hover:shadow-[0_4px_18px_rgba(37,99,235,0.1)] hover:-translate-y-0.5">
          ...
          <div>
            <div className="text-[12px] font-semibold text-slate-400 uppercase tracking-[.06em] mb-1">Your Teams</div>
            <div className="text-[30px] font-extrabold text-slate-900 leading-none">{totalteams}</div>
            <div className="text-[12px] text-slate-400 mt-[3px]">Across your workspace</div>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-[14px] p-[18px_20px] flex items-center gap-4 transition-all duration-[180ms] hover:shadow-[0_4px_18px_rgba(37,99,235,0.1)] hover:-translate-y-0.5">
          ...
          <div>
            <div className="text-[12px] font-semibold text-slate-400 uppercase tracking-[.06em] mb-1">Projects</div>
            <div className="text-[30px] font-extrabold text-slate-900 leading-none">{totalprojects}</div>
            <div className="text-[12px] text-slate-400 mt-[3px]">In progress</div>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-[14px] p-[18px_20px] flex items-center gap-4 transition-all duration-[180ms] hover:shadow-[0_4px_18px_rgba(37,99,235,0.1)] hover:-translate-y-0.5">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[12px] font-semibold text-slate-400 uppercase tracking-[.06em] mb-1 flex items-center justify-between">
              ...
              <span className="text-[11px] font-bold bg-blue-50 text-blue-600 px-[7px] py-px rounded-full">{totaltask.length} total</span>
            </div>
            <div className="text-[30px] font-extrabold text-slate-900 leading-none">{totaltask.filter(t => t.status !== "completed").length}</div>
            <div className="text-[12px] text-slate-400 mt-[3px]">Pending tasks</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-3.5 min-h-0" style={{ gridTemplateColumns: '1fr 1fr 290px' }}>
        ...
        <div className="bg-white border border-slate-200 rounded-[14px] flex flex-col overflow-hidden min-h-0">
          <div className="px-[18px] py-3.5 border-b border-slate-50 flex items-center justify-between shrink-0">
            ...
            <Link href="/dashboard/projects" className="text-[13px] font-semibold text-blue-600 no-underline hover:underline">View all →</Link>
          </div>
          <div className="overflow-y-auto flex-1 min-h-0 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full">
            {projects.length === 0 ? (
              <div className="text-center p-6 text-sm text-slate-400">No projects yet</div>
            ) : projects.map((project) => {
              const palette = ["#2563eb","#7c3aed","#0891b2","#059669","#d97706","#db2777"];
              let h = 0; for (const c of project._id) h = c.charCodeAt(0) + ((h << 5) - h);
              const color = palette[Math.abs(h) % palette.length];
              const prog = taskprogress(project._id);
              return (
                <Link key={project._id} href={`/dashboard/projects/${project._id}`} className="flex items-center gap-3 px-[18px] py-3 border-b border-slate-50 last:border-b-0 no-underline transition-colors duration-[120ms] hover:bg-slate-50 group">
                  <div className="w-10 h-10 rounded-[10px] flex items-center justify-center text-[13px] font-extrabold text-white shrink-0" style={{ background: color }}>
                    {project.name.slice(0,2).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-slate-900 truncate group-hover:text-blue-600 transition-colors">{project.name}</div>
                    <div className="text-xs text-slate-400 mt-0.5">{taskfilter(project._id)} tasks · {getRelativeTime(project.createdAt)}</div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width:`${prog}%`, background: color }} />
                    </div>
                    <span className="text-xs font-bold text-slate-500 w-9 text-right shrink-0">{prog}%</span>
                  </div>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
                    <path d="M9 5l7 7-7 7"/>
                  </svg>
                </Link>
              );
                  ...
                  {displayTasks.length > 0 ? (
                    displayTasks.map((task) => {
                      ...
                      if (due) {
                        ...
                        if (diffDays < 0) {
                          dateLabel = `Overdue · ${formatted}`; dateColor = "#dc2626"; isOverdue = true;
                        } else if (diffDays === 0) {
                          dateLabel = `Due Today · ${formatted}`; dateColor = "#ea580c";
                        } else if (diffDays === 1) {
                          dateLabel = `Tomorrow · ${formatted}`; dateColor = "#d97706";
                        } else if (diffDays <= 7) {
                          dateLabel = `${diffDays}d left · ${formatted}`; dateColor = "#2563eb";
                        } else {
                          dateLabel = formatted; dateColor = "#64748b";
                        }
                      }

                      const dotColor = isOverdue ? "#ef4444" : task.status === "in progress" ? "#f59e0b" : "#94a3b8";
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-slate-800 truncate group-hover:text-blue-600 transition-colors m-0">{task.title}</p>
                          </div>
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <span className="text-xs font-semibold" style={{ color: dateColor }}>{dateLabel}</span>
                          </div>
                        </Link>
                      );
                    })
                  ) : (
                    <div className="text-center p-6 text-sm text-slate-400">🎉 No pending tasks</div>
                  )}
                  {remaining > 0 && (
        ...
        <div className="flex flex-col gap-3.5 min-h-0">
          ...
          {/* Your Teams — fixed max-height, internal scroll */}
          <div className="bg-white border border-slate-200 rounded-[14px] flex flex-col overflow-hidden shrink-0 max-h-[200px]">
            <div className="px-[18px] py-3.5 border-b border-slate-50 flex items-center justify-between shrink-0">
              <span className="text-[15px] font-bold text-slate-900">Your Teams</span>
              <Link href="/dashboard/teams" className="text-[13px] font-semibold text-blue-600 no-underline hover:underline">View all →</Link>
            </div>
            <div className="overflow-y-auto flex-1 min-h-0 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full">
              {teamnames.length === 0 ? (
                <div className="text-center p-6 text-sm text-slate-400">No teams yet</div>
              ) : teamnames.map((team) => {
                const palette = ["#2563eb","#7c3aed","#0891b2","#059669","#d97706","#db2777"];
                let h = 0; for (const c of team._id) h = c.charCodeAt(0) + ((h << 5) - h);
                const color = palette[Math.abs(h) % palette.length];
                return (
                  <Link key={team._id} href={`/dashboard/teams/${team._id}`} className="flex items-center gap-3 px-4 py-2.5 border-b border-slate-50 last:border-b-0 no-underline transition-colors duration-[120ms] hover:bg-slate-50 group">
                    <div className="w-9 h-9 rounded-[9px] flex items-center justify-center text-xs font-extrabold text-white shrink-0" style={{ background: color }}>
                      {team.name.slice(0,2).toUpperCase()}
                    </div>
                    <span className="text-sm font-semibold text-slate-900 flex-1 truncate group-hover:text-blue-600 transition-colors">{team.name}</span>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
                      <path d="M9 5l7 7-7 7"/>
                    </svg>
                  </Link>
                );
          ...
          {/* Recent Activity — grows to fill, internal scroll */}
          <div className="bg-white border border-slate-200 rounded-[14px] flex flex-col overflow-hidden flex-1 min-h-0">
            <div className="px-[18px] py-3.5 border-b border-slate-50 flex items-center justify-between shrink-0">
              <span className="text-[15px] font-bold text-slate-900">Recent Activity</span>
            ...
            <div className="overflow-y-auto flex-1 min-h-0 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-200 [&::-webkit-scrollbar-thumb]:rounded-full">
              {activities && activities.length > 0 ? (
                [...activities]
                  ...
                  .map((activity) => {
                    ...
                    if (actionLower.includes("created") || actionLower.includes("added")) {
                      iconBg = "#dcfce7"; iconColor = "#16a34a"; iconPath = "M12 4v16m8-8H4";
                    } else if (actionLower.includes("completed") || actionLower.includes("done")) {
                      iconBg = "#dbeafe"; iconColor = "#2563eb"; iconPath = "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z";
                    } else if (actionLower.includes("updated") || actionLower.includes("changed") || actionLower.includes("moved")) {
                      iconBg = "#fef9c3"; iconColor = "#ca8a04"; iconPath = "M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z";
                    } else if (actionLower.includes("deleted") || actionLower.includes("removed")) {
                      iconBg = "#fee2e2"; iconColor = "#dc2626"; iconPath = "M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16";
                    }

                    const initials = `${activity.userId?.firstName?.[0] || ""}${activity.userId?.lastName?.[0] || ""}`.toUpperCase() || "??";

                    return (
                      <Link href={`/dashboard/projects/${activity.projectId?._id}`} key={activity._id} className="flex items-start gap-2.5 px-4 py-2.5 border-b border-slate-50 last:border-b-0 no-underline transition-colors duration-[120ms] hover:bg-slate-50">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-px" style={{ background: iconBg }}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] text-slate-500 leading-[1.45] m-0">
                            <strong className="text-slate-900 font-bold">{activity.userName}</strong>{" "}
                            <span>{activity.action}</span>
                            {activity.taskId?.title && (
                              <> <span className="text-blue-600 font-semibold">&quot;{activity.taskId.title}&quot;</span></>
                            )}
                          </p>
                          <div className="flex items-center gap-1.5 mt-[3px]">
                            <div className="w-4 h-4 rounded-full flex items-center justify-center shrink-0" style={{ background: 'linear-gradient(135deg,#3b82f6,#8b5cf6)' }}>
                              <span className="text-[7px] font-extrabold text-white">{initials}</span>
                            </div>
                            <span className="text-[11.5px] text-slate-400">{getRelativeTime(activity.createdAt)}</span>
                ...
                <div className="text-center p-6 text-sm text-slate-400">
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mx-auto mb-1.5">
